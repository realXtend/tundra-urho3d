...
namespace Tundra
{
    namespace Benchmark
    {
        ...
        {
            double msecs = ticks * 1000.0 / math::Clock::TicksPerSec();
        ...
        struct Result
        {
            Result() : iterations(0), fastestTime(0.0), averageTime(0.0), worstTime(0.0), fastestCycles(0.0) {}
...
{ \
    ...
    { \
        unsigned long long startTsc = math::Clock::Rdtsc(); \
        tick_t start = math::Clock::Tick();
        // USER CODE GOES HERE

#define BENCHMARK_STEP_END \
        unsigned long long endTsc = math::Clock::Rdtsc(); \
        tick_t end = math::Clock::Tick(); \
