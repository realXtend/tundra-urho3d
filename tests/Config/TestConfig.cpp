...
namespace 
{
    ...
    static int RandomInt()              { return Random.Int(); }
    static float RandomFloat()          { return Random.Float(); }
    static bool  RandomBool()           { return Random.Int(0,1) == 1; }

    const Variant Value_VAR_INT         = RandomInt();
    const Variant Value_VAR_BOOL        = RandomBool();
    const Variant Value_VAR_FLOAT       = Random.Float();
    const Variant Value_VAR_VECTOR2     = float2::RandomDir(Random);
    const Variant Value_VAR_VECTOR3     = float3::RandomDir(Random);
    const Variant Value_VAR_VECTOR4     = float4::RandomDir(Random);
    const Variant Value_VAR_QUATERNION  = Quat::RandomRotation(Random);
    const Variant Value_VAR_COLOR       = Urho3D::Color(RandomFloat(), RandomFloat(), RandomFloat(), RandomFloat());
    const Variant Value_VAR_STRING      = "This [is] a = a test } { @ ?!";
    // const Variant Value_VAR_BUFFER;
    // const Variant Value_VAR_VOIDPTR;
    // const Variant Value_VAR_RESOURCEREF = Urho3D::ResourceRef(StringHash("TestName"), "TestType");
    // const Variant Value_VAR_RESOURCEREFLIST;
    // const Variant Value_VAR_VARIANTVECTOR;
    // const Variant Value_VAR_VARIANTMAP;
    const Variant Value_VAR_INTRECT     = Urho3D::IntRect(RandomInt(), RandomInt(), RandomInt(), RandomInt());
    const Variant Value_VAR_INTVECTOR2  = Urho3D::IntVector2(RandomInt(), RandomInt());
    // const Variant Value_VAR_PTR;
    const Variant Value_VAR_MATRIX3     = float3x3::RandomGeneral(Random, -RandomFloat(), RandomFloat());   
    const Variant Value_VAR_MATRIX3X4   = float3x4::RandomGeneral(Random, -RandomFloat(), RandomFloat());
    const Variant Value_VAR_MATRIX4     = float4x4::RandomGeneral(Random, -RandomFloat(), RandomFloat());
}
