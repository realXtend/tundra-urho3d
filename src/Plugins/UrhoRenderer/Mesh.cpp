...
float3 Mesh::BonePosition(const String& name) const
{
    ...
    return bone ? float3(bone->GetPosition()) : float3::zero;
}

float3 Mesh::BoneDerivedPosition(const String& name) const
{
    Urho3D::Node* bone = BoneNode(name);
    return bone ? float3(mesh_->GetNode()->GetWorldTransform().Inverse() * bone->GetWorldPosition()) : float3::zero;
}

float3 Mesh::BoneWorldPosition(const String& name) const
{
    Urho3D::Node* bone = BoneNode(name);
    return bone ? float3(bone->GetWorldPosition()) : float3::zero;
}

Quat Mesh::BoneOrientation(const String& name) const
{
    Urho3D::Node* bone = BoneNode(name);
    return bone ? Quat(bone->GetRotation()) : Quat::identity;
}

Quat Mesh::BoneDerivedOrientation(const String& name) const
{
    Urho3D::Node* bone = BoneNode(name);
    return bone ? Quat(mesh_->GetNode()->GetWorldRotation().Inverse() * bone->GetWorldRotation()) : Quat::identity;
}

Quat Mesh::BoneWorldOrientation(const String& name) const
{
    Urho3D::Node* bone = BoneNode(name);
    return bone ? Quat(bone->GetWorldRotation()) : Quat::identity;
}
