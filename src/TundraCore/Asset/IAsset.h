...
public:
    ...
    void SetDiskSource(const String &diskSource);
    ...
    String DiskSource() const { return diskSource; }
    ...
    virtual AssetPtr Clone(String newAssetName) const;
    ...
    virtual bool SaveToFile(const String &filename, const String &serializationParameters = "") const;

    /// Copies the disk cache version of this asset to the specified file. 
    bool SaveCachedCopyToFile(const String &filename);
    ...
    virtual Vector<AssetReference> FindReferences() const { return Vector<AssetReference>(); }

    /// Returns true if the replace succeeds.
    //bool ReplaceReference(const String &oldRef, const String &newRef);

    /// Returns all the assets this asset refers to, and the assets those assets refer to, and so on.
    Vector<AssetReference> FindReferencesRecursive() const;
    ...
    virtual bool SerializeTo(Vector<u8> &data, const String &serializationParameters = "") const;
